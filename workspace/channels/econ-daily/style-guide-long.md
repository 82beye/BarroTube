...
```
MEDIUM framing (slightly zoomed in compared to the Shorts baseline but NOT a close-up): the full body of the character is still visible head-to-feet, character positioned in the upper-center and occupying about 45-50% of the frame height, with generous open space around and below for background props (small arrows, coin stacks, stars, simple charts). Scene style: simple cartoon, flat monochrome with strong warm orange (#F4A261) accent on ONE key prop only. Background color is ONE uniform flat color that continues all the way from the top edge to the bottom edge — do NOT add a different-colored strip, band, ground, floor, or horizon line at the bottom of the frame (the whole background stays the same single color top to bottom). No gradients, no shading, no photorealism. Absolutely no text, letters, words, or labels anywhere. 16:9 horizontal aspect ratio, subject in upper-center.
```
